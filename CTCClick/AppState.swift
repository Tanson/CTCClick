...
//        return false
    ...
    @MainActor
    func saveCommonDir(_ url: URL) {
        let commonDir = CommonDir(id: UUID().uuidString, name: url.lastPathComponent, url: url, icon: "folder.fill")
        cdirs.append(commonDir)
        try? saveCommonDir()
    }
    
    @MainActor
    func removeCommonDir(_ dir: CommonDir) {
        if let index = cdirs.firstIndex(of: dir) {
            cdirs.remove(at: index)
            try? saveCommonDir()
        }
    }
