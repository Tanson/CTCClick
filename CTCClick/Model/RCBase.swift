...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "CTCClick", category: "folder_item")
...
struct NewFile: RCBase {
    ...
    var openApp: URL?
    var template: URL?
    var customIconPath: String? // 新增：自定义图标路径

    init(ext: String, name: String, enabled: Bool = true, idx: Int, icon: String = "document", id: String = UUID().uuidString, customIconPath: String? = nil) {
        self.ext = ext
        self.name = name
        self.enabled = enabled
        self.idx = idx
