...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "CTCClick", category: "FinderOpen")
...
class FinderSyncExt: FIFinderSync {
    ...
    override var toolbarItemToolTip: String {
        return "CTCClick: Click the toolbar item for a menu."
    }
    ...
    @MainActor override func menu(for menuKind: FIMenuKind) -> NSMenu {
        ...
        let applicationMenu = NSMenu(title: "CTCClick")
    ...
    @objc func createMenuForToolbar(_ applicationMenu: NSMenu) {
        ...
        // 添加设置菜单项（如果启用了右键菜单显示）
        if UserDefaults.group.showContextMenu {
            if let settingsMenuItem = createSettingsMenuItem() {
...
//
    ...
    @objc func createSettingsMenuItem() -> NSMenuItem? {
        let menuItem = NSMenuItem()
        menuItem.target = self
        menuItem.title = "设置"
        menuItem.action = #selector(openSettings(_:))
        menuItem.toolTip = "打开设置"
        menuItem.image = NSImage(systemSymbolName: "gearshape", accessibilityDescription: "Settings")
        return menuItem
    }
    
    @MainActor @objc func openSettings(_ menuItem: NSMenuItem) {
        // 发送消息给主应用打开设置窗口
        messager.sendMessage(name: "openSettings", data: MessagePayload(action: "openSettings"))
    }
        ...
        for item in enabledFiletypeItems {
            ...
            if let app = item.openApp {
                ...
                if let customIconPath = item.customIconPath,
                   ...
                   let customImage = NSImage(contentsOfFile: customIconPath) {
                    menuItem.image = customImage
                    menuItem.image?.isTemplate = true
                } else if !item.icon.starts(with: "icon-") {
